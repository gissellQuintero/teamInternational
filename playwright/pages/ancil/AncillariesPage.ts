...
import locators  from './AncillariesPageLocator';
...
export class AncillariesPage extends commonPage{
    ...
    public async selectAdditionalBaggage(flowName:string,screenshotTitle:string,compare:boolean) {
        ...
        const covid = this.testPage.getValue("Covid");
        ...
        await this.page.click(locators.BTN_Continue_Ancillaries);
        } 
    ...
    public async selectTTTOption(flowName: string, screenshotTitle: string, compare: boolean) {
        if (compare) 
        {
            await this.compareScreenshot(screenshotTitle);
        } 
