...
/*
const flowName = 'Test_03_SP_CO_RT_BookingPNR';
...
test.describe(flowName, () => {
    ...
    test('Avail page select default fare', async() => {
        ...
        await availPage.selectFlightsByDefault(flowName,'TC03-availBounds',compare);
    });

    test('Trip summary check for bound cards', async() => {
        const availPage = new AvailabilityPage(testPage,page);
        await availPage.validateTripInfo(flowName,'TC03-TripSummary',compare);
    });

    test('Passenger page fill basic information', async() =>{
        const paxPage = new PassengerPage(testPage,page);
        await paxPage.fillOutPassengerInformation(flowName,'TC03-PassengerInformation',compare);
        await paxPage.enterSpecialFeatures(flowName,'TC03-SpecialServices',compare);
    })

    test('Passenger page fill contact information', async() =>{
        const paxPage = new PassengerPage(testPage,page);
        await paxPage.enterContactInfo(flowName,'TC03-ContactInformation',compare);
       
    })

    test('Ancillaries page Personalize your flight', async() => {
        const ancilPage = new AncillariesPage(testPage,page);
        await ancilPage.selectAdditionalBaggage(flowName,'TC03-BuyExtraBaggage',compare);
        
    });

    test('PURC page - Fill out Card Information', async() => {
        const purcPage = new PurchasePage(testPage,page);
        await purcPage.enterCardDetails(flowName,'TC03-CardInformation',compare);
    });

    test('CONF page', async() =>{
        const confPage = new ConfirmationPage(testPage,page);
        await confPage.validatePNR(flowName,'TC03-ConfirmationPage',compare)
    });
